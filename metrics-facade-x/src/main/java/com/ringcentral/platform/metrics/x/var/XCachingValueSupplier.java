...
public class XCachingValueSupplier<V> implements Supplier<V> {
    ...
    private static final Logger logger = getLogger(XCachingValueSupplier.class);

    public XCachingValueSupplier(CachingVarConfig config, Supplier<V> valueSupplier) {
        this(config, valueSupplier, SystemTimeNanosProvider.INSTANCE);
    }

    public XCachingValueSupplier(
