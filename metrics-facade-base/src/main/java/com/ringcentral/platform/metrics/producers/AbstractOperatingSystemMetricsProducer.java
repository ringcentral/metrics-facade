...
public abstract class AbstractOperatingSystemMetricsProducer extends AbstractMetricsProducer implements OperatingSystemMetricsProducer {
    ...
    protected void produceNonDimensional(MetricRegistry registry) {
        ...
        registry.stringVar(nameWithSuffix("arch"), osMxBean::getArch, stringVarConfigBuilderSupplier(ARCH_DESCRIPTION));
        registry.longVar(nameWithSuffix("availableProcessors"), () -> (long) osMxBean.getAvailableProcessors(), longVarConfigBuilderSupplier(AVAILABLE_PROCESSORS_DESCRIPTION));
        registry.longVar(nameWithSuffix("committedVirtualMemorySize"), osMxBean::getCommittedVirtualMemorySize, longVarConfigBuilderSupplier(COMMITTED_VIRTUAL_MEMORY_SIZE_DESCRIPTION));

        registry.stringVar(nameWithSuffix("name"), osMxBean::getName, stringVarConfigBuilderSupplier(OS_NAME_DESCRIPTION));

        registry.doubleVar(
                nameWithSuffix("systemLoadAverage"),
                osMxBean::getSystemLoadAverage, doubleVarConfigBuilderSupplier(SYSTEM_LOAD_AVERAGE_DESCRIPTION));

        registry.longVar(nameWithSuffix("processCpuTime"), osMxBean::getProcessCpuTime, longVarConfigBuilderSupplier(PROCESS_CPU_TIME_DESCRIPTION));

        if (osMxBean instanceof UnixOperatingSystemMXBean) {
            final UnixOperatingSystemMXBean unixMbean = (UnixOperatingSystemMXBean) osMxBean;
            registry.longVar(nameWithSuffix("descriptor", "file", "open", "total"), unixMbean::getOpenFileDescriptorCount, longVarConfigBuilderSupplier(OPEN_FILE_DESCRIPTOR_COUNT_DESCRIPTION));
            registry.longVar(nameWithSuffix("descriptor", "file", "limit", "total"), unixMbean::getMaxFileDescriptorCount, longVarConfigBuilderSupplier(FILE_DESCRIPTOR_LIMIT_DESCRIPTION));
